...
class Moip_Client
{
    ...
    public function send($credentials, $xml, $url = 'https://desenvolvedor.moip.com.br/sandbox/ws/alpha/EnviarInstrucao/Unica', $method = 'POST')
    {
        ...
        return new Moip_Response(array('resposta' => $ret, 'erro' => $err));
    }
    ...
    function curlPost($credentials, $xml, $url, $error = null)
    {
        ...
        if (!$error)
        {
            ...
            if ($info['http_code'] == "200")
                return new Moip_Response(array('response' => true, 'error' => null, 'xml' => $ret));
            else if ($info['http_code'] == "500")
                return new Moip_Response(array('response' => false, 'error' => 'Error processing XML', 'xml' => null));
            else if ($info['http_code'] == "401")
                return new Moip_Response(array('response' => false, 'error' => 'Authentication failed', 'xml' => null));
            else
                return new Moip_Response(array('response' => false, 'error' => $err, 'xml' => null));
        } else
        {
            return new Moip_Response(array('response' => false, 'error' => $error, 'xml' => null));
        }
    }
